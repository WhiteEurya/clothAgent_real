    ...
    def propose_action(
        ...
        *,
        ...
        prompt = (
            ...
            "rather than a separable free ply; the diagnostic is triage only, not proof. For an "
            "ambiguous candidate, make the first post-grasp move a near-vertical hold, then use "
            "only a short lateral probe if a hanging patch is expected. Do not commit to a long "
            "transport until the hold shows independent material motion.\n"
    ...
    def evaluate(
        ...
        *,
        ...
        prompt = (
            ...
            "If the hold does not show independent hanging material, do not mark the intended "
            "structure as acquired, and do not treat a long lateral transport as tested.\n\n"
            f"Semantic state:\n{json.dumps(semantic_state, ensure_ascii=False, indent=2)}\n\n"
            f"Strategy:\n{json.dumps(strategy.as_dict(), ensure_ascii=False, indent=2)}\n\n"
