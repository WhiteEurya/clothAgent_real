    ...
    def plan_strategy(
        ...
        *,
        ...
        prompt = (
            ...
            "A sharp height change can be either a separable overlapping flap or a rolled wrinkle "
            "left by an earlier action. Keep those hypotheses separate; select a semantic region "
            "for local testing, not a high point as if height alone proved a free ply.\n"
    ...
    def propose_action(
        ...
        *,
        ...
        prompt = (
            ...
            "graspability_score, and surface_shape_diagnostic. Token/semantic confidence is not "
            "graspability. A narrow ridge/spike or mixed occlusion edge may be a rolled wrinkle "
            "rather than a separable free ply; the diagnostic is triage only, not proof. If "
            "surface_shape_diagnostic.compression_probe_recommended is true, close at roughly "
            "recommended_press_below_surface_mm below the candidate's measured surface, then "
            "lift nearly vertically before any lateral motion. Compare the compressed peak with "
            "its neighbours: if the height difference collapses without an independent hanging "
            "patch, classify it as COMPRESSIBLE_SINGLE_PEAK/rolled wrinkle, lower graspability "
            "confidence, release, and re-plan. For an ambiguous candidate, make the first "
            "post-grasp move a near-vertical hold, then use only a short lateral probe if a "
            "hanging patch is expected. Do not commit to a long transport until the hold shows "
            "independent material motion.\n"
            "Capabilities: move(x,y,z,yaw), open_gripper(), close_gripper(), home(). "
            "The move yaw is relative to calibrated Home TCP yaw; yaw=0 preserves "
            "the gripper orientation and avoids a pre-grasp wrist turn.\n\n"
    ...
    def evaluate(
        ...
        *,
        ...
        prompt = (
            ...
            "is supplied. No other measurement names are available. A high ridge is ambiguous: "
            "a peelable overlapping layer should form a visible tent/hanging patch while far "
            "landmarks stay put; a rolled wrinkle may only get taller, narrower, or tighter. "
            "If a compression probe is visible and the peak-to-neighbour height difference "
            "decreases without a hanging patch, call it COMPRESSIBLE_SINGLE_PEAK rather than "
            "successful acquisition. If the hold does not show independent hanging material, "
            "do not mark the intended structure as acquired, and do not treat a long lateral "
            "transport as tested.\n\n"
            f"Semantic state:\n{json.dumps(semantic_state, ensure_ascii=False, indent=2)}\n\n"
            f"Strategy:\n{json.dumps(strategy.as_dict(), ensure_ascii=False, indent=2)}\n\n"
