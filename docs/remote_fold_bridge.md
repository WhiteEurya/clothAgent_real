...
抓取的主判断现在在动作结束、释放并 Home 后，回到标定的 **perception 位姿**重新拍照，再与本轮动作前在同一位姿取得的 Camera A RGB 对比。每轮重新拍动作前图，不复用可能已过时的首轮 perception 图片。Claude 必须返回 `perception_comparison`：`UNCHANGED` / `CHANGED` / `UNCOMPARABLE`，以及置信度和依据。比较衣服的位置、轮廓、袖子形态、折痕与重叠，忽略微小曝光、噪声和对齐差异；相同面积不代表衣服位置没有变化。

按用户指定的任务规则，明确 `UNCHANGED` 时主机记录 `grasp_acquisition=FAILURE`、`earliest_failure_stage=ACQUISITION`，保持当前折叠步骤并要求调整抓取。失败类别为 `NO_CHANGE_AFTER_RETURN`，不会因抬升图难以判断而进入“两次 GRASP_UNOBSERVABLE”停止条件。`CHANGED` 不自动等于抓取成功，`UNCOMPARABLE` 不会被当作未变化。该规则同样用于修复和可逆探测，所以抓住后又放回原位也可能按未抓到处理；这是最终状态判据，不学习成“画面没变必然证明夹爪为空”。`perception_comparison.json` 保存前后图片路径、比较结论和最终抓取判定；原始模型评价另行保留。
...
抬升照片通过 RGB 白名单与 before/after、视频关键帧一起送入最后的 evaluation，作为滑脱与动作过程的辅助证据；同 perception 位姿的最终前后对比是主判据。腕部视角随机械臂运动，异步取得的图片可能已经包含搬运动作，不能标成“运输前静止图”；画面位移或夹爪闭合不等于抓取成功。已到释放完成之后的迟到帧标记为 `MISSED_WINDOW`，不当作抬升抓取证据上传；拍照失败记录 `FAILED`，均不改变已校验轨迹的继续执行。辅助图缺失或不明确不会覆盖清楚可比的最终 UNCHANGED 结论。原有夹爪硬件反馈等待和本地运动校验保留。
...
- 抓取探测在闭爪后高度仍满足至少 30 mm 时保存 Cam A 图片到 `lift_checkpoints/`，包括回退阶段，送给执行后评价作辅助；正式折叠使用最终 perception 前后对比，没有中途视觉关卡。无需额外相机。
