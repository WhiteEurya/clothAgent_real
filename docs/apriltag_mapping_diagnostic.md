...
3. 默认打开实时 RGB 预览，可在移动腕部时查看当前视角。保持静止后，在预览窗口按 Enter/空格，或在终端按 Enter，连续采集三张。采集和分析期间预览暂停，完成后恢复。脚本自动检测板子、保存 RGB/深度/关节角、计算误差。预览窗口按 q/Esc 或关闭窗口结束；无图形桌面时使用 `--no-preview` 保留终端交互。离线 `--session` 模式不打开相机或预览。
